...
const Star = () => {
    ...
    return (
        <div style={{ width: '100vw', height: '100vh', opacity: opa / 100, position: 'relative' }}  className='bg-black'  >
            <Canvas
            ...
            <motion.div
                className="absolute top-1 flex justify-center w-full text-white text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-loadFont whitespace-nowrap"
                style={{ y: textSpring }}
                initial={{ opacity: 0 }}
            ...
            <motion.div
                ...
                animate={{ opacity: showText ? 1 : 0 }}
                transition={{ duration: 0.5 }}
                // style={{ pointerEvents: "none" }} // so it doesn't block scrolling
            >
                <Canvas frameloop={showText ? "always" : "never"}>
                    <ambientLight intensity={0.8} />
                    <directionalLight position={[5, 10, 5]} intensity={1.5} castShadow />
                    <pointLight position={[-5, -5, 10]} intensity={1.2} />
                    <spotLight position={[0, 15, 10]} angle={0.3} intensity={1.3} penumbra={0.5} />
                    <TexturedCylinder visible={showText} />
                    <EffectComposer>
                        <Bloom intensity={6.5} mipmapBlur luminanceThreshold={0.05} luminanceSmoothing={0.15} />
                    </EffectComposer>
                </Canvas>
            </motion.div>
